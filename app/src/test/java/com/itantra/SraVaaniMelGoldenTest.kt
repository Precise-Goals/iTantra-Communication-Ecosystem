...
/**
 * T78 Step 2 — asserts extractLogMelSpectrogram(nMels = 128) matches SraVaani's real
 * preproc.pt-based preprocessor to ~1e-3, the same way MelFeatureGoldenTest (T29) does for
 * IndicConformer's 80-mel path.
 *
 * fixture_sravaani.wav and golden_features_sravaani.npy (app/src/test/resources/) come from a
 * real FLEURS hi_in clip (the same clip validated end-to-end against decode_rnnt in T78 Step 1)
 * run through the actual SraVaaniProcessor from the model's own preproc.pt in Colab — see
 * docs/evaluation/sravaani/tdt/README.md for the exact steps and confirmed params (preemph=0.97,
 * n_fft=512, hop_length=160, win_length=400, log_zero_guard_value=5.960464477539063e-08,
 * normalize=per_feature, CONSTANT=1e-05, pad_value=0.0 — identical to IndicConformer's except
 * the mel-bin count).
 */
class SraVaaniMelGoldenTest {
    ...
    @Test
    fun testExtractLogMelSpectrogramMatchesSraVaaniGoldenReference() {
        val audio = readWavFloatSamples(resourceFile("fixture_sravaani.wav"))
        val (goldenShape, golden) = readNpyFloat32(resourceFile("golden_features_sravaani.npy"))
        val nMels = goldenShape[0]
        val nFrames = goldenShape[1]

        val sttModule = STTModule(mock(Context::class.java), fakeCallbacks)
        val features = sttModule.extractLogMelSpectrogram(audio, nMels = STTModule.SRAVAANI_N_MELS)

        assertTrue(
            "Expected golden shape's mel count (${goldenShape[0]}) to equal STTModule.SRAVAANI_N_MELS " +
                "(${STTModule.SRAVAANI_N_MELS})",
            nMels == STTModule.SRAVAANI_N_MELS
        )
        assertTrue(
        ...
        assertTrue(
            "Worst absolute difference from SraVaani's golden reference is $worstDiff at flat index " +
