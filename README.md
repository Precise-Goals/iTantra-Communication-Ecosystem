...
Uses the real **Silero VAD** ONNX model, downloaded at runtime and stored locally as `silero_vad_v4.onnx` for historical naming reasons (the actual file is pinned to GitHub release tag `v6.2.3` — SHA-256 verified, not trust-on-first-download). It's a v5+ export (`input`/`state`/`sr` tensors, `state` shape `[2,1,128]`), which expects each 512-sample window prefixed with the **previous window's last 64 samples** (576 samples total). That context was missing in earlier builds, which pushed the model's output down to near-zero for every input — including real speech — rather than saturating it high; the model never crossed the speech threshold, so the app permanently fell back to energy-only detection; with the context wired in, `process()` also keeps the loudest of the ~3 32ms sub-windows per 100ms chunk and applies release hysteresis (fires at 0.5, releases at 0.35) so the detector doesn't chatter mid-word. A real RMS-energy detector (fixed 0.025 threshold) remains as an automatic, honest fallback — used only if the model file is missing or a session throws at runtime — so PTT capture is never silently deaf. On a first install, the service's own startup can race the VAD model's download, so `initialize()` finds no file yet and falls back to `BASIC_ENERGY` for the rest of that process — a real gap that only a force-stop used to fix. `VADModule.reinitializeIfNeeded()` now re-runs `initialize()` once the VAD pack finishes downloading, watched from the ViewModel as soon as the service binds, upgrading to `NEURAL` with no restart — verified on a real fresh install (`docs/latency-evidence/run3/receiver_logcat_t73_first_install.txt`).

### Speech-to-Text — `STTModule` + `CtcDecoder`
Uses **AI4Bharat IndicConformer**, exported as per-language sherpa-onnx ONNX INT8 graphs (`stt_{lang}_int8.onnx` + `stt_{lang}_tokens.txt`). Each language is lazy-loaded and cached in a 2-entry LRU (each session is a ~197MB native allocation; since the walkie-talkie's language is user-selectable, nothing previously unloaded a session once you'd tried a third language). `SessionOptions`: 2 intra-op threads, `ALL_OPT`, attempts NNAPI delegate with a CPU/XNNPACK fallback.

The cache's *object count* is verifiably bounded — confirmed on real hardware across 12 evictions in one session, never more than 2 sessions open (see [`docs/latency-evidence/README.md`](docs/latency-evidence/README.md) "Run 3"). The *process's* measured memory footprint (`dumpsys meminfo` TOTAL PSS) was not flat across repeated switches in that same run, though its growth rate was clearly decelerating rather than linear — plausibly native allocator fragmentation, not a per-switch leak, but not yet fully characterized. Read Run 3's T46/T47 section before citing "RAM stays flat" as a proven claim.
...
While PTT is held, each phrase is cut, transcribed, and transmitted independently instead of waiting for the button to be released. `STTModule` inference and model loading are serialized behind a non-reentrant `Mutex`, since the feature extractor reuses shared scratch buffers and would otherwise corrupt itself under concurrent phrases (`TTSModule` synthesis is now serialized the same way, so two messages arriving during a cold voice load can't both trigger a load). Both the STT and TTS models for the selected language are warmed as soon as the app binds to the service, and again on a language change, so the first PTT phrase of a session doesn't pay a model-load stall.

**What's verified, with committed evidence** ([`docs/latency-evidence/`](docs/latency-evidence/) — raw logcat + telemetry from two real two-phone runs, each tied to a specific commit and APK): the receiving device does receive and start speaking phrases sent mid-hold, not just on release, and the NEURAL Silero VAD backend (not the energy fallback) correctly cuts phrases against real human speech while PTT is still held. Received phrases play strictly in order.
...
**Per-language walkie-talkie (T72):** a language chip picker on the Transceiver screen now drives both STT and TTS for PTT messages, not just the AI Assistant (previously every message was recognised and spoken in Hindi regardless of what was selected or spoken). Verified on real hardware for Tamil and Kannada STT.

**The receiver now speaks in the text's own language (T43), not its own locally-selected one.** Run 2 found the receiver mispronouncing every Kannada/Tamil message in the Hindi voice, because `onTextReceived` synthesized with its own `ttsLanguage` instead of the message's actual `srcLang`. Fixed and verified on real hardware in Run 3: a Tamil message to a Hindi-only receiver now correctly produces no audio (`tts_ms=0.0`, no fabricated `[hi]` synthesis) rather than a wrong-language mispronunciation — the honest behaviour until a Tamil voice exists. One caveat: the resulting "TTS not available" error has no logged or on-screen trace yet (`ITantraForegroundService.errorFlow` has no consumer) — see `docs/latency-evidence/README.md`'s "Run 3" section.
...
- Diagnosed and repaired a broken Silero VAD (missing the v5+ export's required 64-sample inter-window context, which had pushed its output to near-zero for every input — including real speech — forcing a permanent energy-only fallback), then pinned the model to a GitHub release tag instead of a floating branch
- Built phrase-level pipelining so phrases spoken mid-hold are cut and transmitted without waiting for PTT release — adaptive endpointing, a Mutex-serialized STT pipeline, and a serialized receiver playback queue — verified against real human speech on two physical devices, with raw logcat/telemetry evidence committed to the repo ([`docs/latency-evidence/`](docs/latency-evidence/))
